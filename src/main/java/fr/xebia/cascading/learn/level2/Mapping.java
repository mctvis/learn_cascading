...
public class Mapping {
	...
	public static FlowDef filterWithExpression(Tap<?, ?, ?> source, Tap<?, ?, ?> sink) {
		Pipe pipe=new Pipe("hello");
		pipe = new Each(pipe,new Fields("line"),new ExpressionFilter("!line.contains(\"Hadoop\")",String.class));
	...
	public static FlowDef transformWithExpression(Tap<?, ?, ?> source, Tap<?, ?, ?> sink) {
		...
		pipe=new Each(pipe,new Fields("line"),new ExpressionFunction(new Fields("line"),"line.toLowerCase()",String.class),Fields.REPLACE);
		return FlowDef.flowDef().addSource(pipe,source).addTailSink(pipe,sink);
	}
	...
	public static FlowDef transformWithACustomFunction(Tap<?, ?, ?> source,
			Tap<?, ?, ?> sink) {
		Pipe pipe=new Pipe("inPipe");
		pipe = new Each(pipe, new Fields("line"), new CustomSplitFunction(new Fields("word")), Fields.SWAP);
